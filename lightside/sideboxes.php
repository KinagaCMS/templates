...
if ($recent_files || $glob_info_files)
{
	...
	'<a href="'. $url. '">'. $n.
	(!is_file('images/logo.png') ? '<img class=mt-3 src="'. $url. 'images/icon.php" alt=logo width=250 height=200><span class="d-block h1 mt-4">'. $site_name. '</span>' : get_logo()). $n.
	'</a>'. $n.
	'</div>';
}
...
if ($recent_files)
{
	...
	'<h5 class=border-0>'. $category. '</h5>'. $n.
...
if ($use_recents && $recent_files)
{
	...
	$aside .= '<div id=recents class="p-4"><h5 class=border-0>'. $recents. '</h5><ul class=list-unstyled>'. $n;
	$j = 0;
...
if ($glob_info_files || $dl || $use_contact && $mail_address)
{
	...
	$aside .= '<div id=informations class="p-4"><h5 class=border-0>'. $informations. '</h5><ul class=list-unstyled>'. $n;
...
if ($address)
	...
	'<h5 class=border-0>'. ($address_title ? $address_title : $site_name). '</h5>'. $n.
...
if ($use_popular_articles && $number_of_popular_articles > 0 &&$glob_all_counter_files = glob($glob_dir. 'counter.txt', GLOB_NOSORT))
{
	...
	'<h5 class=border-0>'. $popular_articles. '</h5><ul class=list-unstyled>'. $n;
...
if ($use_comment && $number_of_new_comments > 0 && $glob_all_comment_files = glob($glob_dir. 'comments/*-~-*.txt', GLOB_NOSORT))
{
	...
	'<h5 class=border-0>'. $recent_comments. '</h5>';
