...
echo
$bootstrap,
file_get_contents('fancybox.css'), file_get_contents('simple-sidebar.css'),
(!$color ? '' : ':root{--bs-primary-rgb:'. implode(',', hsl2rgb($h, $s, $l)). '}'), 'html{scroll-behavior:smooth;text-decoration:none}
...
@media(max-width:1200px){.card-columns{column-count:1}#form .input-group-text{display:block}}
a:hover img,a:hover svg,.social:hover,img[alt=K]{opacity:.8}img[alt=K]:hover{opacity:1}
a{text-decoration:none}
aside a{color:inherit}
aside div:not(:first-child){border-top:1px solid rgba(0,0,0,.1)}
aside #toc .h2{font-size:1.25rem;border:0}
aside .bg-light.current{background-color:inherit!important}
...
{
	list(, $height) = getimagesize($header_img);
	echo'
body:before{background-image:url('. $header_img. ');background-repeat:no-repeat;background-size:cover;content:"";display:block;height:'. $height. 'px;width:100%}
