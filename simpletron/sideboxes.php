...
if ($use_recents && $recent_files = glob($glob_dir. 'index.html', GLOB_NOSORT))
{
	...
	$aside .= '<div id=recents class="mb-5"><h2 class=h5>'. $recents. '</h2>'. $n;
	$j = 0;
	foreach($recent_files as $recents_name)
	{
		...
		$aside .= '<a'. ($categ_name. $title_name === $recent_categ. $recent_title ? ' class=current' : ''). ' href="'. $url. r($recent_categ. '/'. $recent_title). '">'. h($recent_title). '</a>'. $n;
		++$j;
	}
	$aside .= '</div>';
}
...
if ($glob_info_files || $dl || $use_contact && $mail_address)
{
	...
	$aside .= '<div id=informations class="mb-5"><h2 class=h5>'. $informations. '</h2>'. $n;

	foreach($glob_info_files as $info_files)
	{
		if ($info_files === 'contents/index.html') continue;
		$info_uri = basename($info_files, '.html');
		$aside .= '<a'. ($page_name === $info_uri ? ' class=current' : ''). ' href="'. $url. r($info_uri). '">'. h($info_uri). '</a>'. $n;
	}

	if ($dl)
		$aside .= '<a'. ($page_name === $download_contents ? ' class=current' : ''). ' href="'. $url. r($download_contents). '">'. $download_contents. '</a>'. $n;

	if ($use_contact && $mail_address)
		$aside .= '<a'. ($page_name === $contact_us ? ' class=current' : ''). ' href="'. $url. r($contact_us). '">'. $contact_us. '</a>'. $n;
	$aside .= '</div>';
}
...
if ($use_popular_articles && $number_of_popular_articles > 0 &&$glob_all_counter_files = glob($glob_dir. 'counter.txt', GLOB_NOSORT))
{
	...
	'<h2 class=h5>'. $popular_articles. '</h2>'. $n;
	...
	for($i = 0, $c = count($counter_sort); $i < $c && $i < $number_of_popular_articles; ++$i)
	{
		...
		$aside .=
		'<a'. ($categ_name. $title_name === $popular_titles[1]. $popular_titles[2] ? ' class=current' : ''). ' href="'. $url. r($popular_titles[1]. '/'. $popular_titles[2]). '">'. h($popular_titles[2]). '</a>'. $n;
	}
	$aside .= '</div>'. $n;
}
